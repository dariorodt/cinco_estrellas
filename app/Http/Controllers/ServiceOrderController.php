...
class ServiceOrderController extends Controller
{
	...
	public function store(Request $request)
	{
		...
		$service_order = ServiceOrder::create([
			...
			'ending_date' => date('Y-m-d', strtotime(str_replace('/', '-', substr($request->daterange, -10)))),
	...
	public function hire(ServiceOrder $order, Worker $worker)
	{
		...
		$days = $starting_date->diffInDays(Carbon::create($order->ending_date)) + 1;
		...
		$express_percentage = $this->Config->express_percentage;
		$amount = $order->is_express? round($cost * $days * (1+($express_percentage/100))) : round($cost * $days);
